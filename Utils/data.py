...
def create_patches(grid, bands, timestamps):
    ...
    # When bands are passed in, selects the weather array at 1 band (timestamps,band,features)
    if len(bands) > 0:
...
def get_data_loader(grid, batch_size, bands, timestamps):
    ...
    Args:
        ...
        bands - Specificed bands of the weather to take into account
    ...
    '''
    image_patches, weather_patches, label_patches = create_patches(grid, bands, timestamps)
